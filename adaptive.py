...
class AdaptiveHead:
    ...
    def predict(self, features, top_k=1, update_thresholds=True):
        ...
        update_thresholds : bool
            Whether to update homeostatic thresholds during prediction.
            Set to False for evaluation to avoid corrupting thresholds.
        ...
        for i in range(N):
            ...
            if class_sims:
                ...
                # Homeostatic threshold update — only during training/adapt, not eval
    ...
    def accuracy(self, features, labels):
        ...
        Does NOT update homeostatic thresholds — pure evaluation.
        """
    ...
    def add_class(self, name, features, verbose=True):
        ...
        # Initialise threshold to mean of existing thresholds (not 0.0)
        # so new class doesn't get an unfair advantage over calibrated classes
        if self.thresholds:
            self.thresholds[name] = float(np.mean(list(self.thresholds.values())))
        else:
            self.thresholds[name] = 0.0

        # Scale win_counts so new class starts at fair win rate
        # target rate = 1/n_classes, so set win_counts proportionally
        n_classes_after = len(self.prototypes)
        target_wins = self.total_predictions / max(n_classes_after, 1)
        self.win_counts[name] = target_wins
