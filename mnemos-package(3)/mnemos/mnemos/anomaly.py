...
class AnomalyDetector:
    ...
    def process(self, sample, lr=0.01):
        ...
        # ── LAYER 3: DECISION (adaptive threshold + final call) ──
        ...
        # Adaptive threshold: tighten during stable periods,
        # widen during noisy periods
        if len(self._score_buffer) >= 20:
            recent_scores = list(self._score_buffer)[-50:]
            recent_std = float(np.std(recent_scores))
            # If recent scores are stable (low std), tighten threshold
            # If recent scores are noisy (high std), widen threshold
            target = self.threshold + 0.5 * (recent_std - self._train_dist_std)
            self._adaptive_threshold += 0.01 * (target - self._adaptive_threshold)
        ...
        # Online adaptation (only from confident normals)
        adapted = False
        if self.adapt and not is_anomaly and conf < 0.3:
            sims = -raw_dists
            mean_sim = sims.mean()
            specific = sims - mean_sim - self.thresholds
            winner = int(np.argmax(specific))
